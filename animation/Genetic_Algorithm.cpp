...
int output_to_file(vector<int> best_vec, int itnum)
{
	std::stringstream s;
	std::string line = "best_vectors_";
	int l = itnum;
	...
	for (int j = 0; j < 21; j++) { outFile << best_vec[j] << " "; }
	//for (const auto &e : best_vec) { outFile << e << " "; }
	outFile << endl;
